...
Partial Public Class WFP_CausaliNonEvasione
    ...
    Private Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If (Not IsPostBack) Then
            lblMessUtente.Text = ""
        End If
        WUC_CausaliNonEvasione.WucElement = Me
    End Sub

    Protected Sub btnAggiorna_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        If Session(SWOPLOC) = SWOPNUOVO Then
            If CheckNewNumeroOnTab(CheckNewNumeroOnTab(WUC_CausaliNonEvasione.GetNewCodice)) = False Then
                Exit Sub
            End If
        End If
        Session(SWOPLOC) = SWOPNESSUNA
        If (WUC_CausaliNonEvasione.Aggiorna()) Then
            ProgrammaticModalPopup.Hide()
            _WucElement.CallBackWFPCausNonEvasione()
    ...
    Private Function CheckNewNumeroOnTab(ByVal _ID As Int32) As Boolean
        ...
        Dim strSQL As String = "Select Codice From CausNonEvasione WHERE Codice = " & _ID.ToString.Trim
    ...
    Protected Sub btnCancel_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Session(SWOPLOC) = SWOPNESSUNA
        ProgrammaticModalPopup.Hide()
        WUC_CausaliNonEvasione.SvuotaCampi()
        _WucElement.CancBackWFPCausNonEvasione()
        Session(F_ANAGRCAUSNONEV_APERTA) = False
    End Sub
    Protected Sub btnNuovo_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        WUC_CausaliNonEvasione.SetNewCodice(GetNewCodice)
    End Sub
    Private Function GetNewCodice() As Long
        Dim strSQL As String = ""
        strSQL = "Select MAX(ISNULL(Codice,0)) AS Codice From CausNonEvasione"
        Dim ObjDB As New DataBaseUtility
        Dim ds As New DataSet
        Try
            ObjDB.PopulateDatasetFromQuery(TipoDB.dbSoftAzi, strSQL, ds)
