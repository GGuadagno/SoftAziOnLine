...
Partial Public Class WUC_ClientiOCPregr
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Session(CSTTIPODOCOC) = SWTD(TD.OrdClienti)
        ...
        End If
        '-
        If (sTipoUtente.Equals(CSTMAGAZZINO)) Or Session("StampaSCEC") = 1 Then
        ...
        SqlDSOrdTElenco.ConnectionString = dbCon.getConnectionString(TipoDB.dbSoftAzi)
        ...
        If (Not IsPostBack) Then
            Try
                ...
                If SWRbtnTD.Trim = "" Or SWRbtnTD.Trim <> SWTD(TD.OrdClienti) Then
                    Session(CSTSTATODOCSELOC) = "0"
                    Session(CSTSWRbtnTDOC) = SWTD(TD.OrdClienti)
                End If
                '-----------
                'GIU080319
                Dim strStatoDoc As String = Session(CSTSTATODOCSELOC)
                ...
                'giu231221 dipende da clienti e non da qui Session(SWOP) = SWOPNESSUNA
                If strStatoDoc = "" Then
                    Session(CSTSTATODOCOC) = "0"
                    rbtnDaEvadere.Checked = True
                ElseIf strStatoDoc = "1" Then
                    Session(CSTSTATODOCOC) = strStatoDoc
                ...
                ' ''Try
                ' ''    If GridViewOrdT.Rows.Count > 0 Then
                ' ''        Dim savePI = Session("PageIndex")
                ' ''        Dim saveSI = Session("SelIndex")
                ' ''        GridViewOrdT.Sort(Session("SortExp"), Session("SortDir"))
                ' ''        GridViewOrdT.PageIndex = savePI 'Session("PageIndex")
                ' ''        GridViewOrdT.SelectedIndex = saveSI 'Session("SelIndex")
                ' ''        ImpostaFiltro()
                ' ''        GridViewOrdT.DataBind()
                ...
                ' ''    ' ''GridViewOrdT.SelectedIndex = 0
                ' ''    Try
                ' ''        Session(IDDOCOCCLI) = GridViewOrdT.SelectedDataKey.Value
                ' ''        Dim row As GridViewRow = GridViewOrdT.SelectedRow
                ' ''        Dim Stato As String = row.Cells(CellIdxT.Stato).Text.Trim
                ' ''        BtnSetByStato(Stato)
                ' ''        GridViewOrdT_SelectedIndexChanged(GridViewOrdT, Nothing) 'GIU220617
                ' ''    Catch ex As Exception
                ' ''        Session(IDDOCOCCLI) = ""
                ' ''    End Try
                ' ''Else
    ...
    Public Sub CaricaOCCliente()
        ...
        '---------
        ' ''ImpostaFiltro()
        ' ''SqlDSOrdTElenco.DataBind()
        ' ''GridViewOrdT.DataBind()
        ' ''Session(IDDOCOCCLI) = ""
        ' ''SqlDSPrevDByIDDOCOCCLI.DataBind()
        ' ''GridViewPrevD.DataBind()
        ' ''BtnSetEnabledTo(False)
        '' '' ''btnNuovo.Enabled = True
        ' ''If GridViewOrdT.Rows.Count > 0 Then
...
#Region " Ordinamento e ricerca"
    ...
    Private Sub rbtnEvaso_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        btnConfOrdine.Enabled = False
        btnStampa.Enabled = False
        Session(CSTSTATODOCOC) = "1"
        Session(CSTSTATODOCSELOC) = "1"
        BuidDett()
    End Sub
    Private Sub rbtnParzEvaso_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        btnAllestimento.Enabled = True
        btnCreaOF.Enabled = True
        Session(CSTSTATODOCOC) = "2"
        Session(CSTSTATODOCSELOC) = "2"
        BuidDett()
    End Sub
    Private Sub rbtnChiusoNoEvaso_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        btnConfOrdine.Enabled = False
        btnStampa.Enabled = False
        Session(CSTSTATODOCOC) = "3"
        Session(CSTSTATODOCSELOC) = "3"
        BuidDett()
    End Sub
    Private Sub rbtnNonEvadibile_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        btnConfOrdine.Enabled = False
        btnStampa.Enabled = False
        Session(CSTSTATODOCOC) = "4"
        Session(CSTSTATODOCSELOC) = "4"
        BuidDett()
    End Sub
    Private Sub rbtnInAllestimento_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        btnConfOrdine.Enabled = False
        btnStampa.Enabled = False
        Session(CSTSTATODOCOC) = "5"
        Session(CSTSTATODOCSELOC) = "5"
        BuidDett()
    End Sub
    Private Sub rbtnTutti_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        btnAllestimento.Enabled = False
        btnCreaOF.Enabled = True
        Session(CSTSTATODOCOC) = "999"
        Session(CSTSTATODOCSELOC) = "999"
        BuidDett()
    End Sub
    Private Sub rbtnImpegnati_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        Session(CSTSTATODOCOC) = "998"
        Session(CSTSTATODOCSELOC) = "998"
        BuidDett()
    End Sub
    Private Sub rbtnImpegnati100_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        btnCreaOF.Enabled = True
        'giu260617 non più qui ma nella STORE PROCEDURE USANDO I VALORI 998 E 997 IN STATODOC
        Session(CSTSTATODOCOC) = "997"
        Session(CSTSTATODOCSELOC) = "997"
        BuidDett()
    End Sub
    Private Sub rbtnInContratto_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        btnCreaOF.Enabled = True
        'giu260617 non più qui ma nella STORE PROCEDURE USANDO I VALORI 998 E 997 IN STATODOC
        Session(CSTSTATODOCOC) = "996"
        Session(CSTSTATODOCSELOC) = "996"
        BuidDett()
    End Sub
    Private Sub BuidDett()
        ...
        If GridViewOrdT.Rows.Count > 0 Then
            ...
            Try
                ...
                Dim Stato As String = row.Cells(CellIdxT.Stato).Text.Trim
                BtnSetByStato(Stato)
                GridViewOrdT_SelectedIndexChanged(GridViewOrdT, Nothing)
    ...
    Private Sub ImpostaFiltro()
        SqlDSOrdTElenco.FilterExpression = ""
        'GIU240113
        Dim myCoGe As String = Session(COD_CLIENTE) 'GIU220322
        ...
        '---------
        If myCoGe <> "" Then
            SqlDSOrdTElenco.FilterExpression = "Cod_Cliente = '" & Controlla_Apice(myCoGe) & "'"
        Else
            SqlDSOrdTElenco.FilterExpression = "Cod_Cliente = '-1'"
        End If
        ...
        If txtRicerca.Text.Trim <> "" Then
            If ddlRicerca.SelectedValue = "N" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSOrdTElenco.FilterExpression += " AND Numero >= " & txtRicerca.Text.Trim
                Else
                    SqlDSOrdTElenco.FilterExpression += " AND Numero = " & txtRicerca.Text.Trim
                End If
            ...
            ElseIf ddlRicerca.SelectedValue = "D" Then
                If IsDate(txtRicerca.Text.Trim) Then
                    ...
                    If checkParoleContenute.Checked = True Then
                        SqlDSOrdTElenco.FilterExpression += " AND Data_Doc >= '" & CDate(txtRicerca.Text.Trim) & "'"
                    Else
                        SqlDSOrdTElenco.FilterExpression += " AND Data_Doc = '" & CDate(txtRicerca.Text.Trim) & "'"
                    End If
                Else
            ...
            ElseIf ddlRicerca.SelectedValue = "CG" Then 'GIU090212
                If checkParoleContenute.Checked = True Then
                    SqlDSOrdTElenco.FilterExpression += " AND Cod_Cliente like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSOrdTElenco.FilterExpression += " AND Cod_Cliente = '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "R" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSOrdTElenco.FilterExpression += " AND Rag_Soc like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSOrdTElenco.FilterExpression += " AND Rag_Soc >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "C" Then
                If IsDate(txtRicerca.Text.Trim) Then
                    txtRicerca.BackColor = SEGNALA_OK
                    If checkParoleContenute.Checked = True Then
                        SqlDSOrdTElenco.FilterExpression += " AND DataOraConsegna >= '" & CDate(txtRicerca.Text.Trim) & "'"
                    Else
                        SqlDSOrdTElenco.FilterExpression += " AND DataOraConsegna = '" & CDate(txtRicerca.Text.Trim) & "'"
                    End If
                Else
            ...
            ElseIf ddlRicerca.SelectedValue = "V" Then
                If IsDate(txtRicerca.Text.Trim) Then
                    txtRicerca.BackColor = SEGNALA_OK
                    If checkParoleContenute.Checked = True Then
                        SqlDSOrdTElenco.FilterExpression += " AND Data_Validita >= '" & CDate(txtRicerca.Text.Trim) & "'"
                    Else
                        SqlDSOrdTElenco.FilterExpression += " AND Data_Validita = '" & CDate(txtRicerca.Text.Trim) & "'"
                    End If
                Else
            ...
            ElseIf ddlRicerca.SelectedValue = "L" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSOrdTElenco.FilterExpression += " AND Localita like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSOrdTElenco.FilterExpression += " AND Localita >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "M" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSOrdTElenco.FilterExpression += " AND CAP like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSOrdTElenco.FilterExpression += " AND CAP >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "S" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSOrdTElenco.FilterExpression += " AND Denominazione like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSOrdTElenco.FilterExpression += " AND Denominazione >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "P" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSOrdTElenco.FilterExpression += " AND Partita_IVA like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSOrdTElenco.FilterExpression += " AND Partita_IVA >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "F" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSOrdTElenco.FilterExpression += " AND Codice_Fiscale like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSOrdTElenco.FilterExpression += " AND Codice_Fiscale >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "NR" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSOrdTElenco.FilterExpression += " AND Riferimento like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSOrdTElenco.FilterExpression += " AND Riferimento >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "D1" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSOrdTElenco.FilterExpression += " AND Destinazione1 like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSOrdTElenco.FilterExpression += " AND Destinazione1 >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "D2" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSOrdTElenco.FilterExpression += " AND Destinazione2 like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSOrdTElenco.FilterExpression += " AND Destinazione2 >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "D3" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSOrdTElenco.FilterExpression += " AND Destinazione3 like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSOrdTElenco.FilterExpression += " AND Destinazione3 >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            End If
        End If
...
#End Region
    ...
    Private Sub btnVisualizza_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnVisualizza.Click
        ...
        End If
        '-----------
        If Session(SWOPCLI) <> SWOPNESSUNA Or Session(SWOP) <> SWOPNESSUNA Then Exit Sub
        Dim myID As String = Session(IDDOCOCCLI)
        ...
        Session(IDDOCUMENTI) = Session(IDDOCOCCLI) 'giu181221
        Session(CSTTIPODOC) = SWTD(TD.OrdClienti) 'giu161221
    ...
    Private Sub btnNuovoDaOC_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNuovoDaOC.Click
        ...
        Session(MODALPOPUP_CALLBACK_METHOD_NO) = ""
        ModalPopup.Show("Crea nuovo CONTRATTO Cliente da Ordine", "Confermi la creazione CONTRATTO Cliente da Ordine ?<br><br><strong><span>" & myMess & "</span></strong>" & strBloccoCliente, WUC_ModalPopup.TYPE_CONFIRM)
...
#Region " Crea Ordine Fornitore"
    ...
    Public Sub OkCreaNewOF()
        ...
        Try
            ...
            SqlConn.Close()
            'giu181221 Session(IDDOCOCCLI) = SqlDbNewCmd.Parameters.Item("@IDDocumenti").Value
            myID = SqlDbNewCmd.Parameters.Item("@IDDocumenti").Value
            '---------
        Catch ExSQL As SqlException
            Session(MODALPOPUP_CALLBACK_METHOD) = "AvviaRicerca"
            Session(MODALPOPUP_CALLBACK_METHOD_NO) = ""
            ModalPopup.Show("Crea nuovo Ordine FORNITORI", "Verificare i dati dell'ordine appena creato <br><strong><span> " _
        ...
        Try
            'giu181221 assegnato prima myID = Session(IDDOCOCCLI)
            If IsNothing(myID) Then
                myID = ""
            End If
            If String.IsNullOrEmpty(myID) Then
        ...
        'OK FATTO
        Session(SWOP) = SWOPMODIFICA
        Session(IDDOCUMENTI) = myID 'giu181221 assegnato prima Session(IDDOCOCCLI)
...
#End Region
    ...
    Public Sub CreaCopia()
        ...
        Catch ex As Exception
            Session(MODALPOPUP_CALLBACK_METHOD) = ""
            Session(MODALPOPUP_CALLBACK_METHOD_NO) = ""
            ModalPopup.Show("Errore: " + ex.Message.Trim, "NUOVO IDENTIFICATIVO DOCUMENTO SCONOSCIUTO.", WUC_ModalPopup.TYPE_ERROR)
            Exit Sub
        End Try
    ...
    Public Sub NuovaRev()
        ...
        Dim ObjDB As New DataBaseUtility
        Try
            SQLStr = "UPDATE DocumentiT SET StatoDoc=3 WHERE IDDocumenti <> " & myID.Trim 'giu181221 assegnato prima Session(IDDOCOCCLI).ToString.Trim
    ...
    Private Function GetNewRevN(ByVal TDoc As String, ByVal NDoc As String, ByRef NRev As Integer) As Boolean
        If Session(SWOPCLI) <> SWOPNESSUNA Or Session(SWOP) <> SWOPNESSUNA Then Exit Function
        If CKCSTTipoDoc() = False Then
            Chiudi("Errore: TIPO DOCUMENTO SCONOSCIUTO")
            Exit Function
        End If
        ...
        End If
        Dim strSQL As String = ""
        strSQL = "Select MAX(RevisioneNDoc) AS RevisioneNDoc From DocumentiT WHERE Tipo_Doc = '" & Session(CSTTIPODOCOC).ToString.Trim & "'"
    ...
    Private Sub btnElimina_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnElimina.Click
        ...
        Session(IDDOCUMENTI) = Session(IDDOCOCCLI)
        Session(CSTTIPODOC) = SWTD(TD.OrdClienti) 'giu161221
        Response.Redirect("WF_Documenti.aspx?labelForm=Elimina Ordine")
    End Sub

    Private Sub GridViewOrdT_PageIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles GridViewOrdT.PageIndexChanging
        LnkStampa.Visible = False
        If (e.NewPageIndex = -1) Then
            GridViewOrdT.PageIndex = 0
        Else
            GridViewOrdT.PageIndex = e.NewPageIndex
        End If
        GridViewOrdT.SelectedIndex = -1
        Session(IDDOCOCCLI) = ""
        'giu210617
        ...
        End If
    End Sub
    Private Sub GridViewOrdT_Sorted(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridViewOrdT.Sorted
        LnkStampa.Visible = False
        GridViewOrdT.SelectedIndex = -1
        Session(IDDOCOCCLI) = ""
        'giu210617
        ...
        End If
    End Sub
    Private Sub GridViewOrdT_Sorting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSortEventArgs) Handles GridViewOrdT.Sorting
        Session("SortExp") = e.SortExpression
        Session("SortDir") = e.SortDirection
    End Sub
    Private Sub GridViewOrdT_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridViewOrdT.SelectedIndexChanged
        LnkStampa.Visible = False
        Try
            Session("PageIndex") = GridViewOrdT.PageIndex
            Session("SelIndex") = GridViewOrdT.SelectedIndex
    ...
    Private Sub GridViewOrdT_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridViewOrdT.RowDataBound
        'e.Row.Attributes.Add("onclick", Page.ClientScript.GetPostBackEventReference(GridViewOrdT, "Select$" + e.Row.RowIndex.ToString()))
    ...
    Private Sub btnConfOrdine_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnConfOrdine.Click
        ...
        Try
            ...
            If ClsPrint.StampaDocumento(Session(IDDOCOCCLI), TipoDoc, Session(CSTCODDITTA).ToString.Trim, Session(ESERCIZIO).ToString.Trim, TabCliFor, DsPrinWebDoc, ObjReport, SWSconti, StrErrore) Then
                'giu170320
...
#Region " Allestimento"
    ...
    Private Sub btnAllestimento_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAllestimento.Click
        ...
        If strErrore.Trim = "" Then
            ...
            End If
            If swNOCVett Then
                strBloccoCliente += "SENZA Codice Vettore (Non bloccante)<br>"
                swOK = True
            End If
            If swNoDestM Then
                strBloccoCliente += "SENZA Destinazione Merce<br>"
        ...
        ElseIf btnAllestimento.Text = "NO Allestimento" Then
            ...
            Session(MODALPOPUP_CALLBACK_METHOD_NO) = ""
            ModalPopup.Show("Togli Ordine in allestimento", "Confermi la rimozione dell'Ordine selezionato dall'Allestimento ?" & strBloccoCliente & strMessSpese, WUC_ModalPopup.TYPE_CONFIRM)
        End If
    End Sub
...
#Region "Fatturazione Parziale ordine"
    ...
    Public Sub FatturaORParz()
        ...
        If Not IsNumeric(myID) Then
            ...
            Exit Sub
        End If
        Session(IDDOCUMSAVE) = Session(IDDOCOCCLI) 'PER L'EVASIONE PARZIALE
        Session(IDDOCUMENTI) = Session(IDDOCOCCLI) 'GIU150222 ALTRIMENTI NON VISUALIZZA L'ORDINE CORRETTO
        ApriFattORParz()
    End Sub
...
#Region " Crea Fattura"
    ...
    Public Sub CreaFattura()
        ...
        If ListaDocD.Count > 0 Then
            Dim StrSql As String
            Try
                StrSql = "Update DocumentiD Set Qta_Fatturata = 0 Where IDDocumenti = " & Session(IDDOCOCCLI)
            ...
            For I = 0 To ListaDocD.Count - 1
                Try
                    ...
                    QtaDaEv = StrDato(0)
                    NRiga = StrDato(1)
                    StrSql = "Update DocumentiD Set Qta_Fatturata = " & QtaDaEv.ToString.Replace(",", ".") & " Where IDDocumenti = " & Session(IDDOCOCCLI) & " And Riga = " & NRiga
                    ClsDBUt.ExecuteQueryUpdate(TipoDB.dbSoftAzi, StrSql)
                Catch ex As Exception
                    Session(MODALPOPUP_CALLBACK_METHOD) = ""
                    Session(MODALPOPUP_CALLBACK_METHOD_NO) = ""
        ...
        Try
            'giu181221 ass preima myID = Session(IDDOCOCCLI)
            If IsNothing(myID) Then
                myID = ""
            End If
            If String.IsNullOrEmpty(myID) Then
        ...
        Session(IDDOCUMENTI) = myID 'giu181221
...
#Region "Creazione DOCUMENTI COLLEGATI"
    ...
    Private Sub btnDocCollegati_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDocCollegati.Click
        ...
        If String.IsNullOrEmpty(myID) Then
            ...
            Exit Sub
        End If
        Session(IDDOCUMCOLLCALL) = Session(IDDOCOCCLI) 'giu181221
...
#Region "Emissione fattura per ACCONTO/SALDO FORNITURA"
    Private Sub btnFatturaOCAC_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnFatturaOCAC.Click
        ...
        ModalPopup.Show("Emissione Fattura d'Acconto/Saldo", "Confermi l'emissione della fattura d'Acconto/Saldo ?<BR>NOTA, Non sarà evasa alcuna voce dell'ordine.<br>" & strBloccoCliente & strMessSpese, WUC_ModalPopup.TYPE_CONFIRM_YNA)
    End Sub
    ...
    Private Sub CreaNewFCACOR_OK(Optional ByVal SWRecuperaNum As Boolean = False)
        ...
        'OK FATTO
        Session(SWOP) = SWOPMODIFICA
        'GIU201221 LO ASSEGNO QUANDO PASSO ALLA MODIFICA DELLA FATTURA 
        Session(CSTTIPODOC) = SWTD(TD.FatturaCommerciale)
        Session(IDDOCUMENTI) = myID
        Response.Redirect("WF_Documenti.aspx?labelForm=" & strDesTipoDocumento)
    End Sub
    ...
    Private Sub OKApriStampa(ByRef DsPrinWebDoc As DSPrintWeb_Documenti)
        ...
        Try
            If (DsPrinWebDoc.Tables("DocumentiT").Rows.Count > 0) Then
                'giu110319
                Session(CSTTIPODOCOC) = DsPrinWebDoc.Tables("DocumentiT").Rows.Item(0).Item("Tipo_Doc").ToString.Trim
                '---------
            End If
        Catch ex As Exception
            Session(MODALPOPUP_CALLBACK_METHOD) = ""
        ...
        Dim NomeStampa As String = Session(CSTTIPODOCOC)
        ...
        If Session(CSTTIPODOCOC) = SWTD(TD.Preventivi) Then
            ...
            Else
                ...
                End If
            End If
        ElseIf Session(CSTTIPODOCOC) = SWTD(TD.OrdClienti) Then
            ...
            Else
                ...
                '''End If
            End If
        ElseIf Session(CSTTIPODOCOC) = SWTD(TD.DocTrasportoClienti) Then
            ...
            End If
        ElseIf Session(CSTTIPODOCOC) = SWTD(TD.DocTrasportoFornitori) Then
            SubDirDOC = "DDTFornit"
            ...
            ElseIf CodiceDitta = "0501" Then
                Rpt = New DDTNoPrezzi0501
            End If
        ElseIf Session(CSTTIPODOCOC) = SWTD(TD.FatturaCommerciale) Then
            ...
            Else
                ...
                End If
            End If
        ElseIf Session(CSTTIPODOCOC) = SWTD(TD.NotaCredito) Then
            ...
            End If
        ElseIf Session(CSTTIPODOCOC) = SWTD(TD.OrdFornitori) Then
            ...
            End If
        ElseIf Session(CSTTIPODOCOC) = SWTD(TD.MovimentoMagazzino) Or _
                Session(CSTTIPODOCOC) = SWTD(TD.CaricoMagazzino) Or _
                Session(CSTTIPODOCOC) = SWTD(TD.ScaricoMagazzino) Then
            ...
            End If
        ElseIf Session(CSTTIPODOCOC) = SWTD(TD.BuonoConsegna) Or _
            Session(CSTTIPODOCOC) = SWTD(TD.DocTrasportoCLavoro) Or _
            Session(CSTTIPODOCOC) = SWTD(TD.FatturaAccompagnatoria) Or _
            Session(CSTTIPODOCOC) = SWTD(TD.FatturaScontrino) Or _
            Session(CSTTIPODOCOC) = SWTD(TD.NotaCorrispondenza) Then
            Session(MODALPOPUP_CALLBACK_METHOD) = ""
            Session(MODALPOPUP_CALLBACK_METHOD_NO) = ""
            ModalPopup.Show("Attenzione", "Stampa tipodocumento non prevista.: " + Session(CSTTIPODOCOC), WUC_ModalPopup.TYPE_ALERT)
            Exit Sub
        Else
            Session(MODALPOPUP_CALLBACK_METHOD) = ""
            Session(MODALPOPUP_CALLBACK_METHOD_NO) = ""
