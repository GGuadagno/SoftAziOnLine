...
Partial Public Class WUC_NotaCreditoCliente
        ...
        If e.Row.DataItemIndex > -1 Then
            If IsNumeric(e.Row.Cells(CellIdxD.QtaEvIns).Text) Then
                If CDec(e.Row.Cells(CellIdxD.QtaEvIns).Text) <> 0 Then
                    e.Row.Cells(CellIdxD.QtaEvIns).Text = FormattaNumero(CDec(e.Row.Cells(CellIdxD.QtaEvIns).Text), -1).ToString
                Else
...
#Region "Metodi public"
    ...
    Public Function PopolaLista() As Boolean
        ...
        For Each row As GridViewRow In GridViewDocD.Rows
            ...
            If (checkSel.Checked) Then
                ...
                Else
                    ListaDocD.Add(-1 & "|" & row.Cells(CellIdxD.Riga).Text)
                End If
            End If
        Next
        If (ListaDocD.Count > 0) Then
            PopolaLista = True
            Session(L_RESODACF) = ListaDocD
        Else
            PopolaLista = False
        End If
    End Function
    ...
    Public Sub PopolaDocTD()
        ...
        Try
            ...
            SqlDbSelectDocT.CommandText = "get_DocTResiCFByID"
            ...
            SqlDbSelectDocT.Parameters.Item("@IDDocumenti").Value = Session(IDDOCUMENTI).ToString
        ...
        Try
            ...
            '==============CARICAMENTO e MERGE DATASET==============
            SqlDbSelectDocColl.Parameters.Item("@IDDocumenti").Value = Session(IDDOCUMENTI).ToString
        ...
        Catch ExSQL As SqlException
            Session(MODALPOPUP_CALLBACK_METHOD) = ""
            Session(MODALPOPUP_CALLBACK_METHOD_NO) = ""
            ModalPopup.Show("Errore SQL", "Caricamento documento(D): " & ExSQL.Message, WUC_ModalPopup.TYPE_ERROR)
            Exit Sub
        Catch Ex As Exception
            Session(MODALPOPUP_CALLBACK_METHOD) = ""
            Session(MODALPOPUP_CALLBACK_METHOD_NO) = ""
            ModalPopup.Show("Errore", "Caricamento documento(D): " & Ex.Message, WUC_ModalPopup.TYPE_ERROR)
            Exit Sub
        End Try
    End Sub
#End Region
