...
Partial Public Class WUC_ClientiPRPregr
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Session(CSTTIPODOCPR) = SWTD(TD.Preventivi)
        ...
        If (Not IsPostBack) Then
            Try
                ...
                'giu110319
                Dim SWRbtnTD As String = Session(CSTSWRbtnTDPR)
                ...
                If SWRbtnTD.Trim = "" Or SWRbtnTD.Trim <> SWTD(TD.Preventivi) Then
                    Session(CSTSTATODOCSELPR) = "0"
                    Session(CSTSWRbtnTDPR) = SWTD(TD.Preventivi)
                End If
                '-----------
                Session(CSTTIPODOCPR) = SWTD(TD.Preventivi)
                ...
                'GIU080319
                Dim strStatoDoc As String = Session(CSTSTATODOCSELPR)
                ...
                '-----------
                If strStatoDoc = "" Then
                    Session(CSTSTATODOCPR) = "0"
                    rbtnDaConfermare.Checked = True
                ElseIf strStatoDoc = "1" Then
                    Session(CSTSTATODOCPR) = strStatoDoc
                ...
                ' ''    Try
                ' ''        GridViewPrevT.SelectedIndex = 0 'giu201021
                ' ''        Session(IDDocPRCli) = GridViewPrevT.SelectedDataKey.Value
...
#Region " Ordinamento e ricerca"
    ...
    Private Sub rbtnConfermati_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        LnkStampa.Visible = False
        btnCreaOrdine.Enabled = False
        Session(CSTSTATODOCPR) = "1"
        Session(CSTSTATODOCSELPR) = "1"
        BuidDett()
    End Sub
    Private Sub rbtnChiusoNoConf_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        LnkStampa.Visible = False
        btnCreaOrdine.Enabled = False
        Session(CSTSTATODOCPR) = "3"
        Session(CSTSTATODOCSELPR) = "3"
        BuidDett()
    End Sub
    Private Sub rbtnNonConferm_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        LnkStampa.Visible = False
        btnCreaOrdine.Enabled = False
        Session(CSTSTATODOCPR) = "4"
        Session(CSTSTATODOCSELPR) = "4"
        BuidDett()
    End Sub
    Private Sub rbtnTutti_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        LnkStampa.Visible = False
        btnCreaOrdine.Enabled = True
        Session(CSTSTATODOCPR) = "999"
        Session(CSTSTATODOCSELPR) = "999"
        BuidDett()
    End Sub
    ...
    Private Sub ImpostaFiltro()
        ...
        '---------
        If myCoGe <> "" Then
            SqlDSPrevTElenco.FilterExpression = "Cod_Cliente = '" & Controlla_Apice(myCoGe) & "'"
        Else
            SqlDSPrevTElenco.FilterExpression = "Cod_Cliente = '-1'"
        End If
        ...
        If txtRicerca.Text.Trim <> "" Then
            If ddlRicerca.SelectedValue = "N" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSPrevTElenco.FilterExpression += " AND Numero >= " & txtRicerca.Text.Trim
                Else
                    SqlDSPrevTElenco.FilterExpression += " AND Numero = " & txtRicerca.Text.Trim
                End If
            ElseIf ddlRicerca.SelectedValue = "D" Then
                If IsDate(txtRicerca.Text.Trim) Then
                    ...
                    If checkParoleContenute.Checked = True Then
                        SqlDSPrevTElenco.FilterExpression += " AND Data_Doc >= '" & CDate(txtRicerca.Text.Trim) & "'"
                    Else
                        SqlDSPrevTElenco.FilterExpression += " AND Data_Doc = '" & CDate(txtRicerca.Text.Trim) & "'"
                    End If
                Else
            ...
            ElseIf ddlRicerca.SelectedValue = "CG" Then 'GIU090212
                If checkParoleContenute.Checked = True Then
                    SqlDSPrevTElenco.FilterExpression += " AND Cod_Cliente like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSPrevTElenco.FilterExpression += " AND Cod_Cliente = '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "R" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSPrevTElenco.FilterExpression += " AND Rag_Soc like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSPrevTElenco.FilterExpression += " AND Rag_Soc >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "C" Then
                If IsDate(txtRicerca.Text.Trim) Then
                    txtRicerca.BackColor = SEGNALA_OK
                    If checkParoleContenute.Checked = True Then
                        SqlDSPrevTElenco.FilterExpression += " AND DataOraConsegna >= '" & CDate(txtRicerca.Text.Trim) & "'"
                    Else
                        SqlDSPrevTElenco.FilterExpression += " AND DataOraConsegna = '" & CDate(txtRicerca.Text.Trim) & "'"
                    End If
                Else
            ...
            ElseIf ddlRicerca.SelectedValue = "V" Then
                If IsDate(txtRicerca.Text.Trim) Then
                    txtRicerca.BackColor = SEGNALA_OK
                    If checkParoleContenute.Checked = True Then
                        SqlDSPrevTElenco.FilterExpression += " AND Data_Validita >= '" & CDate(txtRicerca.Text.Trim) & "'"
                    Else
                        SqlDSPrevTElenco.FilterExpression += " AND Data_Validita = '" & CDate(txtRicerca.Text.Trim) & "'"
                    End If
                Else
            ...
            ElseIf ddlRicerca.SelectedValue = "L" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSPrevTElenco.FilterExpression += " AND Localita like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSPrevTElenco.FilterExpression += " AND Localita >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "M" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSPrevTElenco.FilterExpression += " AND CAP like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSPrevTElenco.FilterExpression += " AND CAP >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "S" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSPrevTElenco.FilterExpression += " AND Denominazione like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSPrevTElenco.FilterExpression += " AND Denominazione >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "P" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSPrevTElenco.FilterExpression += " AND Partita_IVA like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSPrevTElenco.FilterExpression += " AND Partita_IVA >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "F" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSPrevTElenco.FilterExpression += " AND Codice_Fiscale like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSPrevTElenco.FilterExpression += " AND Codice_Fiscale >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "NR" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSPrevTElenco.FilterExpression += " AND Riferimento like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSPrevTElenco.FilterExpression += " AND Riferimento >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "D1" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSPrevTElenco.FilterExpression += " AND Destinazione1 like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSPrevTElenco.FilterExpression += " AND Destinazione1 >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "D2" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSPrevTElenco.FilterExpression += " AND Destinazione2 like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSPrevTElenco.FilterExpression += " AND Destinazione2 >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            ElseIf ddlRicerca.SelectedValue = "D3" Then
                If checkParoleContenute.Checked = True Then
                    SqlDSPrevTElenco.FilterExpression += " AND Destinazione3 like '%" & Controlla_Apice(txtRicerca.Text.Trim) & "%'"
                Else
                    SqlDSPrevTElenco.FilterExpression += " AND Destinazione3 >= '" & Controlla_Apice(txtRicerca.Text.Trim) & "'"
                End If
            End If
        End If
...
#End Region
    ...
    Private Sub btnVisualizza_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnVisualizza.Click
        ...
        Dim myID As String = Session(IDDOCPRCLI)
        ...
        Session(IDDOCUMENTI) = Session(IDDOCPRCLI)
        Session(CSTTIPODOC) = SWTD(TD.Preventivi) 'giu161221
    ...
    Public Sub CreaOrdine()
        ...
        Catch ExSQL As SqlException
            ...
            ModalPopup.Show("Crea nuovo Ordine da Preventivo", "Verificare i dati dell'ordine appena creato <br><strong><span> " _
                        ...
                        & NDoc.ToString.Trim & "<br> Verificare anche il Preventivo che sia con lo stato Confermato </span></strong><br>" _
        ...
        Session(IDDOCUMENTI) = myID 'giu181221 assegnato prima Session(IDDOCPRCLI)
    ...
    Public Sub CreaCopia()
        ...
        Try
            'giu181221 ass.prima myID = Session(IDDOCPRCLI)
            If IsNothing(myID) Then
                myID = ""
            End If
            If String.IsNullOrEmpty(myID) Then
        ...
        'OK FATTO
        Session(SWOP) = SWOPMODIFICA
        Session(IDDOCUMENTI) = myID.Trim 'giu18121 ass.prima Session(IDDOCPRCLI)
        Session(CSTTIPODOC) = SWTD(TD.Preventivi) 'giu161221
        Response.Redirect("WF_Documenti.aspx?labelForm=Gestione Preventivi/Offerte")
    End Sub
    ...
    Public Sub NuovaRev()
        ...
        Try
            'giu181221 assegn. prima myID = Session(IDDOCPRCLI)
            If IsNothing(myID) Then
                myID = ""
            End If
            If String.IsNullOrEmpty(myID) Then
        ...
        Dim ObjDB As New DataBaseUtility
        Try
            SQLStr = "UPDATE DocumentiT SET StatoDoc=3 WHERE IDDocumenti <> " & myID.Trim 'giu181221 assegnato prima Session(IDDOCPRCLI).ToString.Trim
        ...
        'OK FATTO
        Session(SWOP) = SWOPMODIFICA
        Session(IDDOCUMENTI) = myID.Trim 'giu181221 assegnato primaSession(IDDOCPRCLI)
        Session(CSTTIPODOC) = SWTD(TD.Preventivi) 'giu161221
        Response.Redirect("WF_Documenti.aspx?labelForm=Gestione Preventivi/Offerte")
    End Sub
    ...
    Private Function GetNewRevN(ByVal TDoc As String, ByVal NDoc As String, ByRef NRev As Integer) As Boolean
        ...
        strSQL = "Select MAX(RevisioneNDoc) AS RevisioneNDoc From DocumentiT WHERE Tipo_Doc = '" & Session(CSTTIPODOCPR).ToString.Trim & "'"
    ...
    Private Sub btnStampa_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnStampa.Click
        ...
        Try
            ...
            If ClsPrint.StampaDocumento(Session(IDDOCPRCLI), TipoDoc, Session(CSTCODDITTA).ToString.Trim, Session(ESERCIZIO).ToString.Trim, TabCliFor, DsPrinWebDoc, ObjReport, SWSconti, StrErrore) Then
    ...
    Private Sub OKApriStampa(ByRef DsPrinWebDoc As DSPrintWeb_Documenti)
        ...
        Try
            If (DsPrinWebDoc.Tables("DocumentiT").Rows.Count > 0) Then
                'giu110319
                Session(CSTTIPODOCPR) = DsPrinWebDoc.Tables("DocumentiT").Rows.Item(0).Item("Tipo_Doc").ToString.Trim
                '---------
            End If
        Catch ex As Exception
            Session(MODALPOPUP_CALLBACK_METHOD) = ""
        ...
        If Session(CSTTIPODOCPR) = SWTD(TD.Preventivi) Then
            If SWSconti = 1 Then
                Rpt = New Preventivo
            ...
            Else
                ...
                End If
            End If
        ElseIf Session(CSTTIPODOCPR) = SWTD(TD.OrdClienti) Then
            If SWSconti = 1 Then
                Rpt = New Ordine
                If CodiceDitta = "01" Then
                    Rpt = New Ordine01
            ...
            Else
                ...
                '''End If
            End If
        ElseIf Session(CSTTIPODOCPR) = SWTD(TD.DocTrasportoClienti) Then
            Rpt = New DDTNoPrezzi
            If CodiceDitta = "01" Then
            ...
            End If
        ElseIf Session(CSTTIPODOCPR) = SWTD(TD.DocTrasportoFornitori) Then
            Rpt = New DDTNoPrezzi
            If CodiceDitta = "01" Then
                Rpt = New DDTNoPrezzi01
            ElseIf CodiceDitta = "05" Then
            ...
            ElseIf CodiceDitta = "0501" Then
                Rpt = New DDTNoPrezzi0501
            End If
        ElseIf Session(CSTTIPODOCPR) = SWTD(TD.FatturaCommerciale) Then
            'giu251211
            If SWSconti = 1 Then
                Rpt = New Fattura
                If CodiceDitta = "01" Then
            ...
            Else
                ...
                End If
            End If
        ElseIf Session(CSTTIPODOCPR) = SWTD(TD.NotaCredito) Then
            Rpt = New NotaCredito
            If CodiceDitta = "01" Then
            ...
            End If
        ElseIf Session(CSTTIPODOCPR) = SWTD(TD.OrdFornitori) Then
            ...
            End If
        ElseIf Session(CSTTIPODOCPR) = SWTD(TD.MovimentoMagazzino) Or _
                Session(CSTTIPODOCPR) = SWTD(TD.CaricoMagazzino) Or _
                Session(CSTTIPODOCPR) = SWTD(TD.ScaricoMagazzino) Then
            Rpt = New MMNoPrezzi
            If CodiceDitta = "01" Then
            ...
            End If
        ElseIf Session(CSTTIPODOCPR) = SWTD(TD.BuonoConsegna) Or _
            Session(CSTTIPODOCPR) = SWTD(TD.DocTrasportoCLavoro) Or _
            Session(CSTTIPODOCPR) = SWTD(TD.FatturaAccompagnatoria) Or _
            Session(CSTTIPODOCPR) = SWTD(TD.FatturaScontrino) Or _
            Session(CSTTIPODOCPR) = SWTD(TD.NotaCorrispondenza) Then
            Session(MODALPOPUP_CALLBACK_METHOD) = ""
            Session(MODALPOPUP_CALLBACK_METHOD_NO) = ""
            ModalPopup.Show("Attenzione", "Stampa tipodocumento non prevista.: " + Session(CSTTIPODOCPR), WUC_ModalPopup.TYPE_ALERT)
            Exit Sub
        Else
            Session(MODALPOPUP_CALLBACK_METHOD) = ""
            Session(MODALPOPUP_CALLBACK_METHOD_NO) = ""
...
#Region "Creazione DOCUMENTI COLLEGATI"
    ...
    Private Sub btnDocCollegati_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDocCollegati.Click
        ...
        If String.IsNullOrEmpty(myID) Then
            ...
            Exit Sub
        End If
        Session(IDDOCUMCOLLCALL) = Session(IDDOCPRCLI) 'giu181221
...
#End Region
    ...
    Private Sub btnCambiaStato_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCambiaStato.Click
        ...
        Else
            ...
            Exit Sub
        End If
        Session(IDDOCCAMBIAST) = Session(IDDOCPRCLI) 'giu181221
        WFPCambiaStatoPR.WucElement = Me
