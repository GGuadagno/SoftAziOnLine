...
Partial Public Class WUC_DiffImportoRiga
    ...
    Private Sub StampaReport()
        ...
        Try
            If clsStampa.StampaDiffImportoRiga(TipoDoc, txtDataDa.Text.Trim, txtDataA.Text.Trim, Session(CSTAZIENDARPT), DSDiffImportoRiga1, Errore) Then
                If DSDiffImportoRiga1.DiffImportoRiga.Count <= 0 Then
                    Session(MODALPOPUP_CALLBACK_METHOD) = ""
                    Session(MODALPOPUP_CALLBACK_METHOD_NO) = ""
                    ModalPopup.Show("Controllo", "Non ci sono documenti che abbiano differenza, secondo i criteri selezionati.", WUC_ModalPopup.TYPE_INFO)
                    Exit Sub
                End If
                Session(CSTDsPrinWebDoc) = DSDiffImportoRiga1
                Session(CSTTIPORPTCONTROLLO) = TIPOSTAMPACONTROLLO.DiffImportoRiga
