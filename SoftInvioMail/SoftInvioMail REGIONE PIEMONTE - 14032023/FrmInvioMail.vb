...
Public Class FrmInvioMail
        ...
        If InviaEmail() = True Then
            ...
            Else 'ASPETTO OPERAZIONI DA SOFTAZIENDA eventuali correzioni email e altri tipi di errore
                ...
                If NEmailErrate > 0 Then
                    LblInvio.Text = "ATTESA INVIO EMAIL (Reg.Piemonte)" & NEmailDaInviare.ToString & "/" & NEmailInviate.ToString & " ERR.: " & NEmailErrate.ToString
                Else
                    LblInvio.Text = "ATTESA INVIO EMAIL (Reg.Piemonte)" & NEmailDaInviare.ToString & "/" & NEmailInviate.ToString & " ERR.: " & NEmailErrate.ToString
                End If
                ErrMess = LblInvio.Text
    ...
    Private Function InviaEmail() As Boolean
        ...
        Try
            ...
            SqlStr = "Select * FROM EmailInviateRegPiemonte WHERE ISNULL(Stato,0)=0 OR ISNULL(Stato,0)<0"
            ErrMess = SqlStr
            Call ScriviFileLog(ErrMess)
            myCmd.CommandText = SqlStr
            myCmd.Connection = myCn
            ...
            For Each RowEmailT In DsEmailDaInviare.EmailInviateTOggetto.Select("Stato<>-1", "ID")
                ...
                If ErrInvio = False Then
                    With objEmail
                        ...
                        'OGGETTO
                        .Subject = IIf(InvioTest = True, "!!!TEST INVIO EMAIL!!! - Field Service Notification – INFORMATION REQUIRED ", "Field Service Notification – INFORMATION REQUIRED")
                ...
                If Me.myCn.State = ConnectionState.Closed Then
                    Me.myCn.Open()
                End If
                SqlStr = "UPDATE EmailInviateRegPiemonte SET Stato=" & RowEmailT.Stato.ToString.Trim & ", DataInvio=GETDATE(),[Note]='" & Controlla_Apice(RowEmailT.Note.Trim) & _
