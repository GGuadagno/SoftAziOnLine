...
Public Class FrmInvioMail
        ...
        If InviaEmail() = True Then
            ...
            Else 'ASPETTO OPERAZIONI DA SOFTAZIENDA eventuali correzioni email e altri tipi di errore
                ...
                If NEmailErrate > 0 Then
                    LblInvio.Text = "ATTESA INVIO EMAIL (Pug./Bas.)" & NEmailDaInviare.ToString & "/" & NEmailInviate.ToString & " ERR.: " & NEmailErrate.ToString
                Else
                    LblInvio.Text = "ATTESA INVIO EMAIL (Pug./Bas.)" & NEmailDaInviare.ToString & "/" & NEmailInviate.ToString & " ERR.: " & NEmailErrate.ToString
                End If
                ErrMess = LblInvio.Text
    ...
    Private Function InviaEmail() As Boolean
        ...
        Try
            ...
            SqlStr = "Select * FROM EmailInviateTOggetto WHERE ISNULL(Stato,0)=0 OR ISNULL(Stato,0)<0"
            ErrMess = SqlStr
            Call ScriviFileLog(ErrMess)
            myCmd.CommandText = SqlStr
            myCmd.Connection = myCn
            ...
            For Each RowEmailT In DsEmailDaInviare.EmailInviateTOggetto.Select("Stato<>-1", "ID")
                ...
                Else
                    If RowEmailT.IsEmailNull Then
                        If RowEmailT.IsEmailInvioScadNull Then
                            If RowEmailT.IsEmailInvioFattNull Then
                                'NULLA
                            ElseIf RowEmailT.EmailInvioFatt.Trim <> "" Then
                                Cst_EmailDestinatario = RowEmailT.EmailInvioFatt.Trim
                            End If
                        ElseIf RowEmailT.EmailInvioScad.Trim <> "" Then
                            Cst_EmailDestinatario = RowEmailT.EmailInvioScad.Trim
                        ElseIf RowEmailT.IsEmailInvioFattNull Then
                            'NULLA
                        ElseIf RowEmailT.EmailInvioFatt.Trim <> "" Then
                            Cst_EmailDestinatario = RowEmailT.EmailInvioFatt.Trim
                        End If
                    ElseIf RowEmailT.Email.Trim <> "" Then
                ...
                If ErrInvio = False Then
                    With objEmail
                        ...
                        'OGGETTO
                        .Subject = IIf(InvioTest = True, "!!!TEST INVIO EMAIL!!! - NUOVI RIFERIMENTI COMMERCIALI ", "NUOVI RIFERIMENTI COMMERCIALI")
                        .Body = "<!doctype html><html xmlns='http://www.w3.org/1999/xhtml'> " & _
                                      "<head>" & _
                                      "<title>NUOVI RIFERIMENTI COMMERCIALI</title>" & _
                        ...
                        .Body += "con nostro dispiacere siamo a comunicare la prematura scomparsa del collega Michel Giuliano, referente commerciale per le Regioni Puglia e Basilicata. <br /><br />"

                        .Body += "Vi chiediamo gentilmente per informazioni, offerte e ordini relativamente ai prodotti Philips, Prestan, Corpuls e Mindray, di contattarci ai seguenti recapiti: <br /><br />"

                        .Body += "E-MAIL: <a href=""mailto:" & Cst_EmailMittente.ToLower.Trim & "?Subject=Comunicazione"" target=""_top"">" & Cst_EmailMittente.ToLower.Trim & "</a><br /><br />"
                        .Body += "Telefono: 051 0935879<br />"
                        .Body += "Fax: 051 0935882<br />"

                        .Body += "Vi ringraziamo della Vostra attenzione.<br />"
                        .Body += "Restando a disposizione per ogni eventuale chiarimento, porgiamo cordiali saluti.<br />"
                ...
                If Me.myCn.State = ConnectionState.Closed Then
                    Me.myCn.Open()
                End If
                SqlStr = "UPDATE EmailInviateTOggetto SET Stato=" & RowEmailT.Stato.ToString.Trim & ", DataInvio=GETDATE(),[Note]='" & Controlla_Apice(RowEmailT.Note.Trim) & _
                         "', Destinatario='" & Controlla_Apice(RowEmailT.Destinatario.Trim) & "' WHERE ID=" & RowEmailT.Id.ToString.Trim
                ErrMess = SqlStr
                Call ScriviFileLog(ErrMess)
                myCmd.CommandText = SqlStr
                myCmd.Connection = myCn
