...
Public Class FrmInvioMail
        ...
        If InviaEmail() = True Then
            ...
            Else 'ASPETTO OPERAZIONI DA SOFTAZIENDA eventuali correzioni email e altri tipi di errore
                ...
                If NEmailErrate > 0 Then
                    LblInvio.Text = "ATTESA INVIO EMAIL (MISTA)" & NEmailDaInviare.ToString & "/" & NEmailInviate.ToString & " ERR.: " & NEmailErrate.ToString
                Else
                    LblInvio.Text = "ATTESA INVIO EMAIL (MISTA)" & NEmailDaInviare.ToString & "/" & NEmailInviate.ToString & " ERR.: " & NEmailErrate.ToString
                End If
                ErrMess = LblInvio.Text
    ...
    Private Function InviaEmail() As Boolean
        ...
        Try
            ...
            SqlStr = "Select * FROM EmailInviateMISTA WHERE ISNULL(Stato,0)=0 OR ISNULL(Stato,0)<0"
            ErrMess = SqlStr
            Call ScriviFileLog(ErrMess)
            myCmd.CommandText = SqlStr
            myCmd.Connection = myCn
            ...
            For Each RowEmailT In DsEmailDaInviare.EmailInviateTOggetto.Select("Stato<>-1", "ID")
                ...
                If ErrInvio = False Then
                    With objEmail
                        ...
                        'OGGETTO
                        .Subject = IIf(InvioTest = True, "!!! VERIFICARE SE TUTTO CORRETTO GRAZIE !!! - Field Service Notification – INFORMATION REQUIRED", "Field Service Notification – INFORMATION REQUIRED")
                        .Body = "<!doctype html><html xmlns='http://www.w3.org/1999/xhtml'> " & _
                                      "<head>" & _
                                      "<title>Field Service Notification – INFORMATION REQUIRED</title>" & _
                                      "<meta charset='utf-8'/> " & _
                                      "<meta name='viewport' content='width=device-width, initial-scale=1, maximum-scale=1'/>" & _
                                      "</style>" & _
                                      "</head>" & _
                                      "<body>"
                        .Body += "<b>" + "<span style='color: #FF0000;'>NUMERO DI CODICE CLIENTE </span> " + " 94375750</b><br /><br />"
                        .Body += "Gentile cliente di DAE Philips, <br /><br />"
                        .Body += "recentemente ha ricevuto una lettera, un'e-mail e/o una cartolina per informarla di un problema identificato con gli elettrodi DAE Philips M5071A (pazienti adulti) che può rappresentare un rischio per i pazienti (alleghiamo per comodità). <br />"
                ...
                If Me.myCn.State = ConnectionState.Closed Then
                    Me.myCn.Open()
                End If
                SqlStr = "UPDATE EmailInviateMISTA SET Stato=" & RowEmailT.Stato.ToString.Trim & ", DataInvio=GETDATE(),[Note]='" & Controlla_Apice(RowEmailT.Note.Trim) & _
